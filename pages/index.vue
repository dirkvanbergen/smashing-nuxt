<template>
  <section class="container flex flex-wrap flex-row">
    <img src="@/static/images/smashing-foto-1.jpg" class="-mt-8 md:px-2 w-full h-32 md:h-48 lg:h-64 object-cover object-center mb-4"  />
    <ul v-if="news" class="px-2">
        <li v-for="(post, index) in news" :key="index" class="pb-4">
            ...
            <p><span class="text-smashing pr-1">{{post.sys.createdAt | formatDate}}</span> {{post.fields.content}}</p>
        </li>
    </ul>
  </section>
</template>
